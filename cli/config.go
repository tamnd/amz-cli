	...
	cmd.AddCommand(
		...
		&cobra.Command{
			...
			RunE: func(cmd *cobra.Command, args []string) error {
				...
				_, _ = fmt.Fprintf(w, "rate_floor       = %s (%s under --no-robots)\n", amz.MinDelay, amz.MinDelayNoRobots)
				_, _ = fmt.Fprintf(w, "robots           = obeyed, cached %s\n", amz.RobotsTTL)
				_, _ = fmt.Fprintf(w, "user_agent       = %s\n", amz.UserAgent())
